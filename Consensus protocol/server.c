...
int main(int argc, char *argv[]) {
  ...
  socklen_t src_addr_len = sizeof(src_addr);
	size_t pkt_size = 0;
	while (!quit) {
		if (SERVER_PORT == 5001 || SERVER_PORT == 5002) { // 5001, 5002번 포트
			strcpy(RecvMsg.value,""); // 버퍼 초기화
			n = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), MSG_DONTWAIT, (struct sockaddr *)&src_addr, &src_addr_len);	//sock 소켓으로부터 수신받은 데이터를 RecvMsg 구조체 변수에 RecvMsg 크기만큼 저장.
			if (n > 0) {
				if (RecvMsg.type == READ_REQ) {
					...
					sendto(sock, &RecvMsg, pkt_size, 0, (struct sockaddr *)&src_addr, sizeof(src_addr));//  sock 소켓을 통해 RecvMsg 구조체 변수에 있는 데이터를 RecvMsg 크기만큼 src_addr에(클라이언트) 송신
			}
			}
		}
		else if (SERVER_PORT == 5003){ // 5003번 포트
			strcpy(RecvMsg.value,""); // 버퍼 초기화
			n = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), MSG_DONTWAIT, (struct sockaddr *)&src_addr, &src_addr_len);	//sock 소켓으로부터 수신받은 데이터를 RecvMsg 구조체 변수에 RecvMsg 크기만큼 저장.
			if (n > 0) {
				if (RecvMsg.type == READ_REQ) {
				printf("Type: %s Key: %s Value: %s\n",get_type(RecvMsg),RecvMsg.key, RecvMsg.value); // 수신한 내용을 출력한다.
				RecvMsg.type = READ_REP;
				strcpy(RecvMsg.value, "SSWUCSE");  // KVS 참조를 하지 않고 조건에 맞는 필드값을 넣어준다
				pkt_size = n + VALUE_SIZE;
				sendto(sock, &RecvMsg, pkt_size, 0, (struct sockaddr *)&src_addr, sizeof(src_addr));//  sock 소켓을 통해 RecvMsg 구조체 변수에 있는 데이터를 RecvMsg 크기만큼 src_addr에(클라이언트) 송신
			}
			}
